...
import buildapk.apk_utils as apk_utils
import buildapk.config_utils as config_utils
import buildapk.file_utils as file_utils
import buildapk.change_parameters as change
from channelsdks import game_channel_config

androidNS = 'http://schemas.android.com/apk/res/android'
generate_new_r_file_use_aapt2 = {'sd', 'hlxd'}
recompile_use_new_apktool = {'skzl'}

generate_r_file_use_sharkaapt = {'aaa'}
compile_use_apktool = {}
...
def delete_sign_dir(game, dec_dir, smali_dir):
    ...
    if game not in generate_new_r_file_use_aapt2:
        sign_dir_two = os.path.join(dec_dir, "unknown\META-INF")
...
def pack_all_channels(args):
    ...
    for opt, arg in opts:
        if opt in ("-g", "--game"):
            # game_name = game_channel_config.game_name.get(arg)
            game_name=file_utils.contain_zh(arg)
    ...
    print('channel_args : ', channel_args + ", package_opt :", package_opt+game_name)
    if game_name == "" or channel_args == "-o":
        print('parameter invalide! build quit!')
        return;

    if game_name not in game_channel_config.old_logic_game:
        channels = config_utils.request_channels_params(channel_args, game_name)
    else:
...
def change_icon_if_need(game, channel_name, dec_dir, father_node):
    ...
    channel_icon_file = file_utils.getFullPath("sdks/%s/icon_%s.png" % (game, channel_name))
...
def change_logo_if_need(game, channel_name, dec_dir, father_node):
    ...
    channel_logo_file = file_utils.getFullPath("channelsdks/%s/logo_%s.png" % (channel_name,channel_name))
    if os.path.exists(channel_logo_file):
        ...
        for root, dirs, files in os.walk(seach_dir):  # path 为根目录
            ...
            if "xiaoy_bg.jpg" in files:
                ...
                os.remove(p)
                shutil.copy(channel_logo_file, p)
...
def check_muti_dex(src_dir, dst_dir,game):
    ...
    return apk_utils.jar2smali(src_dir, dst_dir,game)
...
def do_pack(game, channel, source_apk, output_path, package_opt, reinstall):
    ...
    sdk_source_dir = file_utils.getFullPath('channelsdks/%s' % channel_sdk)
    ...
    # instead the lastest plugin，if exist
    if not os.path.exists("sdks/%s/plugin.dat" % game):
        print('user itself plugin')
    else:
    ...
    if not channel_sdk.startswith('stvgame'):
        print("开始遍历")
        apk_utils.jar2dex((sdk_source_dir), sdk_dest_dir,game)
    ...
    change.del_xml_content(manifest_file,channel)
    ...
    if change.change_icon("sdks/%s" % game ,channel_name):
        change_icon_if_need(game, channel_name, dec_dir, father_node)
        file_utils.printF("%s渠道%s的icon已更换",(channel_name,game))

    change_logo_if_need(game, channel_name, dec_dir, father_node)


    check_muti_dex(os.path.join(sdk_source_dir, "todex"), os.path.join(dec_dir, "smali_classes2"),game)
    ...
    if channel_sdk.startswith('stvgame'):
        ...
        if channel_sdk.startswith('tcl'):
            ...
            shutil.copytree(file_utils.getFullPath("channelsdks/%s/aidl/" % (channel_sdk)),
    ...
    apk_utils.change_provider_authorities(dec_dir, package_name)

    if game not in game_channel_config.old_logic_game:
        ret = apk_utils.copy_resource_ifexist(game,sdk_dest_dir, dec_dir, channel_sdk)
    else:
    ...
    # 写入渠道参数
    apk_utils.write_develop_info(channel, dec_dir,game)
    apk_utils.write_support_info(game, channel, dec_dir)
    # 生成R文件
    ret = apk_utils.generate_new_r_file(game, package_name, dec_dir)
    if ret:
        return None

    file_utils.printF("需要配置的值是%s",channel)
    # 处理manifest参数替换
    change.change_manifest_parameters(manifest_file,channel,game)
    # 处理assets参数值替换
    assets_file=dec_dir+"/assets"
    change.change_assets_parameters(assets_file,channel)
