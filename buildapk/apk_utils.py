...
import zipfile
import shutil
from buildapk import new_main as main
import buildapk.file_utils as file_operate
import buildapk.config_utils as config_utils
from xml.etree import ElementTree as ET
from xml.etree.ElementTree import SubElement
from channelsdks import game_channel_config
from buildapk import change_parameters as change
...
def decompile_apk(game, source, target_dir, apktool="apktool_2.4.1.jar"):
    apkfile = file_operate.getFullPath(source)
    target_dir = file_operate.getFullPath(target_dir)

    if game in main.recompile_use_new_apktool:
        apktool = "apktool.jar"
    ...
    cmd = '"%s" -jar "%s" d -f "%s" -o "%s" --only-main-classes' % (
        file_operate.getJavaCMD(), apktool, apkfile, target_dir)

    ret = file_operate.exec_format_cmd(cmd)
    return ret


def recompile_apk(game, source_folder, apk_file, apk_tool="apktool_2.4.1.jar"):
    ...
    # if game in main.compile_use_apktool.keys():
    #   apk_tool = '%s' % (main.compile_use_apktool.get(game) + '.jar')
    ...
    aapt2 = file_operate.getFullToolPath("aapt2.exe")
    ...
    if os.path.exists(source_folder):
        ...
        # cmd = '"%s" -jar "%s" -q b -f "%s" -o "%s" -a "%s"' % (file_operate.getJavaCMD(), apk_tool, source_folder, apk_file,aapt2)
        ret = file_operate.exec_format_cmd(cmd)
    return ret


def change_provider_authorities(decompile_dir, package_name):
    ...
    auth_key = '{' + androidNS + '}authorities'
    name_key = '{' + androidNS + '}name'

    if provider_lst is not None and len(provider_lst) > 0:
        for provider in provider_lst:
            if provider.get(name_key) == 'com.stvgame.ysdk.business.YTSFileProvider':
                provider.attrib[auth_key] = package_name + '.fileprovider'
                print('更改provider为 :' + provider.attrib[auth_key])
...
def copy_resource_ifexist(game, sdk_dir, decompile_dir, channel):
    """
     取消配置，Androidmanifest，res ,lib ,assets ,只要这个路径下面有这个文件，就拷贝到包里
    :param sdk_dir:
    :param decompile_dir:
    :return:
    """
    ...
    if channel == 'stvgame':
        print("  没有资源需要复制或者合并 ")
        return 0
    print('start copy resource %s_%s ' % (sdk_dir, decompile_dir))

    if os.path.exists(sdk_dir.replace('/', '\\') + '\sdk_manifest.xml'):
        print("  打包渠道 " + channel + game)
        manifest_from = file_operate.getFullPath(os.path.join(sdk_dir, 'sdk_manifest.xml'))
        manifest_to = file_operate.getFullPath(os.path.join(decompile_dir, 'AndroidManifest.xml'))
        # print(" manifestFrom is :" + manifest_from + " manifestTo is :" + manifest_to)
        # merge into xml
        manifest_to = manifest_to.replace("\\", '/')
        manifest_from = manifest_from.replace("\\", "/")
        ...
        print('not sdk_mainfest to merge')
    res = []
    with os.scandir(sdk_dir) as dirs:
        for dir in dirs:
            if dir.is_dir():
                res.append(dir.name)

    print('need copy files', res)
    if os.path.exists(sdk_dir) and len(res) > 0:
        for index in range(len(res)):
            copy_from = file_operate.getFullPath(os.path.join(sdk_dir, res[index]))
            copy_to = file_operate.getFullPath(os.path.join(decompile_dir, res[index]))
            if res[index] == 'libs':
                # libs 中的jar不需要 copy
                continue
            copy_from = copy_from.replace("\\", '/')
            copy_to = copy_to.replace("\\", "/")
            copy_res_to_apk(copy_from, copy_to)
            print('copy res include', (copy_from, copy_to))
    return 0
...
def write_develop_info(channel, decompile_dir, game):
    ...
    config_utils.write_developer_properties(channel, develop_config_file, game)


def write_support_info(game, channel, decompile_dir):
    develop_config_file = os.path.join(decompile_dir, "assets")
    if not os.path.exists(develop_config_file):
        os.makedirs(develop_config_file)

    develop_config_file = os.path.join(develop_config_file, "plugin_config.xml")
    if game not in game_channel_config.old_logic_game:
        config_utils.write_plugin_configs_from_plubic(channel, develop_config_file)
    else:
...
def generate_new_r_file(game, new_package_name, decompile_dir):
    ...
    elif game in main.generate_r_file_use_sharkaapt:
        print('user sharkAapt to generate R file')
        aapt_path = file_operate.getFullToolPath("sharkAapt")
        file_operate.printF("The generate R aapt file path is %s", aapt_path)
...
def jar2smali(src_dir, dst_dir,game, dextool="baksmali.jar"):
    ...
    jar2dex(src_dir, dex_dir,game)
    ret = dex2smali(os.path.join(dex_dir, 'classes.dex'), dst_dir)
    return ret


def jar2dex(src_dir, dst_dir,game,dextool="baksmali.jar"):
    ...
    # 私有目录下是否有pay_jar,判断使用私有目录下的jar
    local_path = file_operate.getFullPath('sdks/%s/sdk' % game)
    result = change.copy_private_sdk(local_path)
    if result:
        file_operate.printF("相对路径下有对应的jar  %s", result)
        for f in os.listdir(local_path):
            if f.endswith(".jar"):
                cmd = cmd + " " + os.path.join(local_path, f)
    else:
        file_operate.printF("相对路径下没有对应的jar  %s", result)
